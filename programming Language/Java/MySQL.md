...
对聚合之后的表进行过滤，即上图中的右表
* having和where子句连用
1、WHERE 子句筛选原始数据表中的行。
2、数据表按照 GROUP BY 子句中的列进行分组。
3、聚合函数（如 SUM、COUNT、AVG 等）在分组后的数据上执行。
4、HAVING 子句对聚合函数的结果进行筛选。（对右图进行过滤）
...
1、B+树只有在叶子节点才存放数据，而B树中的每个节点既有索引也有数据
如果对同样数据量建立索引，B+树会比B树更加矮胖，减少平均IO
2、范围查找：通过左边界定位到叶子节点后，B+树能够利用节点之间的双向链表，直接向右查询。B树就要重新从根节点进行查询。减少IO
* 聚簇索引vs二级索引
...
MYISAM中的索引全部都是二级索引，根节点存放的都是排序的列和行数

> InnoDB与MyIsam存储引擎之间的区别：
索引的区别；
事务的区别：InnoDB支持事务，MyIsam不支持
...
2、脏读：读的数据是被其他事务修改后的：读的数据是废数据；事务B往表中写数据value，后来又不想要这个数据了，进行了rollback。但是这个value被事务A读到了。
...
## MVCC
* RepeatableRead如何保证不发生脏读
通过MVCC机制：more specific，版本链中的transactionId+ReadView这一数据结构 来决定某个版本的记录对当前事务是否可见
